...
def evaluation(data_loader, model, clf, args, output_path, file_name):
    ...
    predicted_labels = clf.predict(concatenated_features)
...
if __name__ == '__main__':
    ...
    conf.add_argument("--train_path", type=str, default='The path for the features to train the classifier.')
    ...
    if args.transformation == 'without':
        train_features_path = glob(train_path +"/img_features.npy") 
        train_img_path = glob(train_path +"/img_pathes.npy") 
        train_gt_path = glob(train_path +"/ground_truth.npy")

    elif args.transformation == 'with':
        train_features_path = glob(train_path +"/*/img_features.npy") +  glob(train_path +"/*/*/*/img_features.npy")
        train_img_path = glob(train_path +"/*/img_pathes.npy") +  glob(train_path +"/*/*/*/img_pathes.npy")
        train_gt_path = glob(train_path +"/*/ground_truth.npy") +  glob(train_path +"/*/*/*/ground_truth.npy")
    ...
    for feat, path, gt in zip(train_features_path, train_img_path, train_gt_path):   
        ...
        if len(train_features) == 0:
            ...
            train_gt =  np.hstack((train_gt, ground_truth))
            train_img_paths =  np.hstack((train_img_paths, img_paths))

    if args.task == 'A':
        train_gt_binary = []
        for i in train_gt.tolist():
            if i == 0:
                train_gt_binary.append(0)
            else:
                train_gt_binary.append(1)
        train_gt_binary = np.array(train_gt_binary)
        train_gt = train_gt_binary
    ...
    for idx, test_file in enumerate(test_files):
        ...
        if args.task == 'A':
            accuracy, r_acc, f_acc, f1 = evaluation(test_data_loader, model, clf, args, output_path, file_name)
        elif args.task == 'B':
            accuracy, f1,r_acc, f_acc_sd21, f_acc_sdxl, f_acc_sd3, f_acc_dalle, f_acc_midjourny = evaluation(test_data_loader, model, clf, args, output_path, file_name)
